...
const datasets: DatasetDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "amount", label: "原币金额" },
      { key: "exchangeRate", label: "汇率" },
      { key: "amountCny", label: "折合人民币" },
      { key: "status", label: "状态" },
    ],
  },
  {
          ...
          <TabsList className="flex h-auto min-h-10 w-full flex-wrap items-center justify-start gap-1 rounded-xl bg-muted p-1">
            {DATASET_GROUP_ORDER.map((g) => (
              <TabsTrigger
                key={g}
                value={g}
                className="h-8 flex-none items-center justify-center rounded-lg px-2.5 py-0 text-xs leading-none text-foreground/70 after:hidden hover:!bg-primary hover:!text-amber-200 data-active:!bg-primary data-active:!text-white data-active:shadow-sm data-active:hover:!bg-primary data-active:hover:!text-white dark:hover:!text-amber-200 dark:data-active:!text-white"
              >
                {g.replace(/^M0(\d) /, "M0$1 · ")}
              </TabsTrigger>
            ))}
          ...
          <TabsList className="flex h-auto min-h-10 w-full flex-wrap items-center justify-start gap-1 rounded-xl border bg-card p-1">
            {moduleItems.map((d) => (
              <TabsTrigger
                ...
                value={d.id}
                title={d.desc}
                className="h-8 flex-none items-center justify-center rounded-lg px-3 py-0 text-sm leading-none text-foreground/70 after:hidden hover:!bg-primary hover:!text-amber-200 data-active:!bg-primary data-active:!text-white data-active:shadow-sm data-active:hover:!bg-primary data-active:hover:!text-white dark:hover:!text-amber-200 dark:data-active:!text-white"
              >
                {d.name}
              </TabsTrigger>
            ))}
